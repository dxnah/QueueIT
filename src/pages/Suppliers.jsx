            ...
            <div className="modal-body">
              <form onSubmit={handleSubmit}>
                <div className="modal-field">
                  ...
                  <input type="text" value={form.name} onChange={e => setForm(p => ({...p, name: e.target.value}))} required placeholder="e.g., MedSource Philippines" style={inputStyle} />
                </div>
                <div className="modal-grid-2">
                  <div>
                    <label className="modal-label">Email / Contact</label>
                    <input type="email" value={form.contact} onChange={e => setForm(p => ({...p, contact: e.target.value}))} required placeholder="info@supplier.ph" style={inputStyle} />
                  </div>
                  <div>
                    <label className="modal-label">Phone</label>
                    <input type="text" value={form.phone} onChange={e => setForm(p => ({...p, phone: e.target.value}))} placeholder="+63 2 8234 5678" style={inputStyle} />
                  </div>
                </div>
                <div className="modal-field">
                  <label className="modal-label">Address</label>
                  <input type="text" value={form.address} onChange={e => setForm(p => ({...p, address: e.target.value}))} placeholder="Full address" style={inputStyle} />
                </div>
                <div className="modal-grid-2">
                  <div>
                    <label className="modal-label">Vaccines Supplied (comma-separated)</label>
                    <input type="text" value={form.vaccines} onChange={e => setForm(p => ({...p, vaccines: e.target.value}))} placeholder="Anti-Rabies, Booster" style={inputStyle} />
                  </div>
                  <div>
                    <label className="modal-label">Lead Time (days)</label>
                    <input type="number" min="1" value={form.lead_time_days} onChange={e => setForm(p => ({...p, lead_time_days: e.target.value}))} placeholder="7" style={inputStyle} />
                  </div>
                </div>
                <div className="modal-grid-2 modal-grid-2--mb">
                  ...
                  <div>
                    ...
                    <input type="text" value={form.notes} onChange={e => setForm(p => ({...p, notes: e.target.value}))} placeholder="Optional notes" style={inputStyle} />
                  </div>
                </div>
      ...
      <section className="main-wrapper">
        ...
        <main className="main-content">
          ...
          <div className="page-header" style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:'12px' }}>
            ...
            <button onClick={openAdd} style={{ padding:'10px 20px', borderRadius:'8px', border:'none', background:'#26a69a', color:'white', fontWeight:'700', fontSize:'13px', cursor:'pointer' }}>
              ➕ Add Supplier
            </button>
          </div>
          ...
          <div style={{ display:'flex', gap:'10px', marginBottom:'20px', flexWrap:'wrap', alignItems:'center' }}>
            ...
            {['all', 'Active', 'Inactive'].map(s => (
              <button key={s} onClick={() => setFilterStatus(s)}
                style={{ padding:'7px 16px', borderRadius:'20px', border:`1.5px solid ${filterStatus===s?'#26a69a':'#e0e0e0'}`, background:filterStatus===s?'#26a69a':'white', color:filterStatus===s?'white':'#555', fontSize:'13px', fontWeight:'600', cursor:'pointer' }}>
                {s === 'all' ? 'All' : s}
              </button>
            ))}
          </div>
          ...
          {error && (
            <div style={{ padding:'14px', background:'#ffebee', border:'1.5px solid #ef9a9a', borderRadius:'8px', color:'#c62828', marginBottom:'16px', fontSize:'13px' }}>
              ❌ {error}
            </div>
          )}
                    ...
                    <tbody>
                      {filtered.map((s, i) => (
                        <tr key={s.id} style={{ borderBottom:'1px solid #f0f0f0', background:i%2===0?'#fafafa':'white' }}
                          ...
                          <td style={{ padding:'12px 14px' }}><a href={`mailto:${s.contact}`} style={{ color:'#26a69a', textDecoration:'none' }}>{s.contact}</a></td>
                          ...
                          <td style={{ padding:'12px 14px', color:'#555', textAlign:'center' }}>{s.lead_time_days}d</td>
