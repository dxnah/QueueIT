...
import {
  ...
  CHART_COLORS,
  getMonthMultiplier,
} from '../data/analyticsConstants';
import '../styles/analytics.css';

// ─── Seeded random (deterministic, so charts are consistent on refresh) ───────
const seededRand = (seed) => {
  const x = Math.sin(seed + 1) * 10000;
  return x - Math.floor(x);
};

// ─── Generate hourly dispensing data ─────────────────────────────────────────
            ...
            <AreaChart data={chartData} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
              <defs>
                {keysToShow.map(name => (
                  <linearGradient key={name} id={`grad_${name.replace(/\s/g, '')}`} x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%"  stopColor={VACCINE_COLORS[name]} stopOpacity={0.3} />
                    <stop offset="95%" stopColor={VACCINE_COLORS[name]} stopOpacity={0.02} />
                  </linearGradient>
                ))}
              </defs>
              ...
              {keysToShow.map(name => (
                <Area key={name} type="monotone" dataKey={name}
                  stroke={VACCINE_COLORS[name]} strokeWidth={2}
                  fill={`url(#grad_${name.replace(/\s/g, '')})`}
                  dot={{ r: 3, fill: VACCINE_COLORS[name] }} activeDot={{ r: 5 }} />
              ))}
            </AreaChart>
          </ResponsiveContainer>
        )}
