...
export const mlInsights = [
  {
    ...
    items: [
      ...
      { id: 'a2', label: 'Batch expiry risk',         note: 'Hepatitis B Batch HB-2024-004 expires in 30 days',  status: 'warning' },
      { id: 'a3', label: 'Zero-stock anomaly',        note: 'Booster has been out of stock for 3+ consecutive days', status: 'critical' },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'r2', label: 'Order Hepatitis B soon',  note: 'ML recommends 300 doses — only 2 doses remaining',    status: 'critical' },
      { id: 'r3', label: 'Monitor Anti-Tetanus',    note: 'Consider ordering 350 doses within this week',        status: 'warning' },
    ],
  },
];
...
export const vaccineData = [
  ...
  { id: 2, vaccine: 'Anti-Tetanus', available: 85,  minStock: 200, mlRecommended: 350, status: 'Low Stock', batchNumber: 'AT-2024-002', expiryDate: '2029-08-20' },
  { id: 3, vaccine: 'Booster',      available: 0,   minStock: 150, mlRecommended: 500, status: 'Out Stock', batchNumber: 'BS-2024-003', expiryDate: '2029-04-10' },
  { id: 4, vaccine: 'Hepatitis B',  available: 2,   minStock: 100, mlRecommended: 300, status: 'Low Stock', batchNumber: 'HB-2024-004', expiryDate: '2029-07-25' },
  { id: 5, vaccine: 'Flu Shot',     available: 21,  minStock: 100, mlRecommended: 150, status: 'In Stock',  batchNumber: 'FL-2024-005', expiryDate: '2029-05-30' },
];
...
export const vaccineColorList = ['#26a69a', '#f57f17', '#e53935', '#5c6bc0', '#2e7d32'];

// ─── MONTHLY ORDER REQUIREMENTS ───────────────────────────────────────────────
// Base monthly doses needed per vaccine (normal months).
// Peak months (June, July, August) are automatically multiplied by 1.5.
...
// Returns the required doses for a given vaccine and month
...
// ─── DAILY USAGE LOG ──────────────────────────────────────────────────────────
// Shape: { 'January': { 'Anti-Rabies': { 1: 120, 2: 98, ... }, ... }, ... }
// This is the shared in-memory store. In production this would be a backend DB.
export const dailyUsageLog = {};

// Helper: get total used so far for a vaccine in a month
...
// Helper: log usage for a specific day (adds/replaces that day's entry)
...
// Helper: get urgency level based on remaining doses vs requirement
export const getOrderUrgency = (remaining, required) => {
  if (required === 0) return 'normal';
  const pct = remaining / required;
  if (pct <= 0.15) return 'urgent';   // ≤15% left
  if (pct <= 0.40) return 'soon';     // ≤40% left
  return 'normal';
};
...
export const notificationsData = [
  ...
  { id: 3, type: 'info',     title: 'ML Prediction Update',          message: 'Peak demand expected June–August. Recommended stock: 6,000 doses.',           time: '2 hours ago',   read: false },
